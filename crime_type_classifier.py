...
class CrimeTypeClassifier:
    """Crime type classification service"""
    
    def __init__(self, model_path: str = None):
        if model_path is None:
            self.model_path = Path(__file__).parent / "crime_type_rf_model.pkl"
        else:
            ...
            if self.model_path.exists():
                ...
                if isinstance(loaded_model, dict):
                    if 'model' in loaded_model:
                        ...
                        self.model = loaded_model['classifier']
                    elif 'estimator' in loaded_model:
                        self.model = loaded_model['estimator']
                    else:
    ...
    def classify_crime_types(self, crime_data: pd.DataFrame) -> Dict[str, Union[List[str], Dict[str, float]]]:
        """Classify crime types for given data"""
        try:
            logger.info("Classifying crime types...")
            
            if self.model is None:
                logger.warning("Model not loaded. Using fallback classification based on crime descriptions.")
                return self._fallback_classification(crime_data)
            ...
            if hasattr(self.model, 'classes_'):
                prediction_names = [self.model.classes_[pred] for pred in predictions]
            else:
                prediction_names = predictions.tolist()
    ...
    def _fallback_classification(self, crime_data: pd.DataFrame) -> Dict[str, Union[List[str], Dict[str, float]]]:
        """
        ...
        Returns:
            Dict[str, Union[List[str], Dict[str, float]]]: Fallback classification results
        """
        try:
            # Simple rule-based classification based on crime descriptions
            predictions = []
            probabilities = []
            confidence_scores = []
            ...
            for _, row in crime_data.iterrows():
                ...
                if 'THEFT' in crime_desc or 'STOLEN' in crime_desc:
                    pred = 'THEFT'
                    prob = [0.8, 0.1, 0.1]  # [THEFT, ASSAULT, OTHER]
                    conf = 0.8
                elif 'ASSAULT' in crime_desc or 'BATTERY' in crime_desc:
                    pred = 'ASSAULT'
                    prob = [0.1, 0.8, 0.1]
                    conf = 0.8
                elif 'BURGLARY' in crime_desc or 'BREAK' in crime_desc:
                    pred = 'BURGLARY'
                    prob = [0.1, 0.1, 0.8]
                    conf = 0.8
                else:
            ...
            return {
                "predictions": ["UNKNOWN"] * len(crime_data),
                "probabilities": [[1.0]] * len(crime_data),
                "confidence_scores": [0.0] * len(crime_data),
                "num_predictions": len(crime_data),
    ...
    def analyze_crime_type_trends(self, crime_data: pd.DataFrame) -> Dict[str, Union[List[Dict[str, any]], Dict[str, any]]]:
        """
        Analyze trends in crime types over time.
        ...
        Returns:
            Dict[str, Union[List[Dict[str, any]], Dict[str, any]]]: Trend analysis
                Format: {
                    "trends": [
                        {"crime_type": "THEFT", "trend": "increasing", "change_percentage": 15.5},
                        {"crime_type": "ASSAULT", "trend": "decreasing", "change_percentage": -8.2}
                    ],
                    "statistics": {
        ...
        """
        try:
            logger.info("Analyzing crime type trends...")
            
            # Convert date column
            crime_data['DATE OCC'] = pd.to_datetime(crime_data['DATE OCC'])
            
            # Group by crime type and time period (monthly)
            crime_data['year_month'] = crime_data['DATE OCC'].dt.to_period('M')
            trends_data = crime_data.groupby(['Crm Cd Desc', 'year_month']).size().reset_index(name='count')
            ...
            for crime_type in crime_data['Crm Cd Desc'].unique():
                crime_trend = trends_data[trends_data['Crm Cd Desc'] == crime_type]
                
                if len(crime_trend) >= 2:
                    # Calculate simple trend (first half vs second half)
                    mid_point = len(crime_trend) // 2
                    first_half_avg = crime_trend.iloc[:mid_point]['count'].mean()
                    second_half_avg = crime_trend.iloc[mid_point:]['count'].mean()
                ...
                trends.append({
                    ...
                    "total_occurrences": int(crime_trend['count'].sum()) if len(crime_trend) > 0 else 0
                })
            ...
            logger.info(f"Analyzed trends for {len(trends)} crime types")
            return results
