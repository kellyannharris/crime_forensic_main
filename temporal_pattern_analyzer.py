    ...
    def detect_seasonality(self, period: Optional[int] = None) -> Dict[str, Union[Dict[str, float], List[float]]]:
        ...
        try:
            ...
            if period is None:
                ...
                for p in [7, 30, 365]:  # weekly, monthly, yearly
                    if len(self.time_series_data) >= 2 * p:
                        ...
                        break
                if period is None:
                    period = max(2, len(self.time_series_data) // 4)  # Default to 1/4 of data
            
            # Ensure period is valid
            if period >= len(self.time_series_data):
                period = max(2, len(self.time_series_data) // 2)
            
            # Perform seasonal decomposition
            decomposition = seasonal_decompose(self.time_series_data, period=period, extrapolate_trend='freq')
            self.decomposition = decomposition
            
            # Calculate strength measures with safety checks
            ...
            if total_var == 0:
                ...
                noise_strength = resid_var / total_var
    ...
    def forecast_future(self, periods: int = 30, confidence_level: float = 0.95) -> Dict[str, Union[List[str], List[float]]]:
        ...
        try:
            if self.arima_model is None:
                # Try to fit a simple model if none exists
                logger.warning("No ARIMA model fitted. Fitting a simple model for forecasting...")
                self.fit_arima_model(order=(1, 1, 0))
